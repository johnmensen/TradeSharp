...
struct ConGroup
  {
   ...
   char              signature[256];              // statements signature
...
struct ConManager
  {
   ...
   int               server_reports;              // right to receive automatic server reports
   int               unused[12];
   //--- IP filtration
...
struct CServerInterface
  {
   ...
   //--- special checks
   virtual int          __stdcall TradesCheckSessions(ConSymbol *symbol,const time_t ctm);
...
//+------------------------------------------------------------------+
//--- login types
enum { LOGIN_FLAG_CLIENT=0, LOGIN_FLAG_MOBILE=1, LOGIN_FLAG_MULTI=2, LOGIN_FLAG_DC=3, LOGIN_FLAG_ADMIN=4, LOGIN_FLAG_MANAGER=5, LOGIN_FLAG_MANAPI =6, LOGIN_FLAG_WD=7, LOGIN_FLAG_PHONE=8 };
...
int  APIENTRY        MtSrvUserInterestMonth(const UserRecord *user,const ConGroup *group);
//+------------------------------------------------------------------+
